...
Node *insertBeforeValue(Node *head, int el, int val)
{
 if (head == NULL)
 {
  // ll doesn't exist so can't find value before which the new element node has to be inserted
  return NULL;
 }

 // ll has only one element before which new node has to be inserted
 if (head->data == val)
 {
  return new Node(el, head);
 }
 ...
 Node *temp = head;
 // stopping when we find the last node
 while (temp->next != NULL)
 {

  // temp is the second last node in ll rn, and the last node's data == val, new node to be inserted at second last position
...
int main()
{
 ...
 head = insertBeforeValue(head, 400, 2);
 print(head);
 return 0;
}